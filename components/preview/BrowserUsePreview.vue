...
const props = defineProps<{
  args: {sandboxId: string|null|undefined, task: string};
  result?: {sandboxId:string|null|undefined, vncUrl: string, error: string|null|undefined};
}>();
...
const formattedUrl = computed(() => {
  if (!props.result?.vncUrl) return '';
  return props.result.vncUrl.replace(/\/$/, '');
});
...
<template>
  <div class="preview-url-container p-4">
    ...
    <div v-if="isLoading" class="loading-state py-8 flex flex-col items-center justify-center">
      ...
      <div class="text-gray-600 mb-2">正在生成连接...</div>
      <div class="text-xs text-gray-500">请稍候，正在连接 VNC 服务</div>
    </div>
    ...
    <!-- 成功状态 -->
    <div v-else-if="result?.vncUrl" class="success-state">
      <!-- 桌面vnc -->
      <div
        ...
        <div v-if="!iframeLoaded" class="relative">
          ...
          >
            <div class="flex flex-col items-center">
              ...
              ></div>
              <div class="text-sm text-gray-600">加载桌面中...</div>
            </div>
          </div>
        </div>
